...
- **`category-reconciliation-upgrade-path.mjs` + `category-reconciliation-concurrency.mjs` + `category_reconciliation.test.mjs`**
  ...
  NULL, unknown and ambiguous coverage; exhaustive explicit UUID manifests;
  exact two-code system seeding; no name-join authority; stale evidence and
  late-error atomic rollback; active, deleted, rule, unknown, NULL and Other
