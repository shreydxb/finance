...
Status: canonical implemented direction, updated for the SHR-194 repository package. Statements labeled **current** describe repository implementation. Production is verified through migration `044`; migrations `045`, `046`, `047` and `048` and their backup-manifest changes are repository-only pending independent Tier-3 review and are not applied or deployed.
