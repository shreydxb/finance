...
Status: canonical implemented direction, updated for the SHR-196 repository package. Statements labeled **current** describe repository implementation. Production is verified through migration `044`; merged migration `045` and review migration `046` are not applied or deployed.
...
- Migration `046` adds category lifecycle/protection substrate without changing V1 classification or any consumer. `categories.id` remains identity; `system_code` is nullable and restricted to `transfer|savings_investment`, with no code seed. The database owner is the intentional migration/restore assignment root, while browser/service DML cannot assign a code and no ordinary DML can change or clear one after assignment. Rename, archive/reactivation, row delete, and truncate fail closed. Immutable `category_name_history` is distinct from explicit `category_aliases`; only an exact `compatibility_active` alias reserves a label, while `history_only` and rename history do not. Household members can read both evidence tables through existing membership RLS, but no API role can write them directly. No resolver or lifecycle RPC exists.
...
- SHR-196 does not seed system codes, add transaction/rule stable references, change the exact-name Transfer/Savings classification, filter archived categories, or alter current category consumers. Those remain separately gated under SHR-197/198/160. The existing Settings rename/delete controls will fail closed at the database boundary after a future authorized apply until replacement lifecycle UX is separately implemented.
