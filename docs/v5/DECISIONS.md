...
## ADR-021 — Account ownership is a reviewed economic fact, never a label and never authorization

Status: accepted and implemented in repository migration `049`; independent Tier-3 review, human manifest approval and production apply all pending — 31 August 2026 — SHR-154

ADR-020 decided how an access identity becomes an economic party. This one decides how a financial fact acquires a stable owner, and it applies the same rule to the same failure mode. `accounts.owner` is free text a human typed — production holds `Shrey` and `Tarika`, and the app also offers `Joint`; `Both`, `Me` and `Partner` are v6 presentation language no row carries. None of them is an identity. Two accounts labelled identically may belong to different economic parties or to the household; a `Joint` label is a data-entry habit, not proof of shared ownership; and any label may be edited tomorrow. So `ownership_kind` and `owner_party_id` are populated only by an explicitly approved manifest, and the reconciliation path reads no label, account name, transaction, category, goal, Telegram identity or historical percentage — the 69/31 income target least of all, which has never described account ownership and allocates nothing here.

Three ownership states, and they are three different statements rather than degrees of confidence. `personal` names exactly one economic party by UUID. `household` is genuinely shared truth: one row, counted once, with no owning party. `unreconciled` is the absence of a reviewed fact and is the default every existing account carries — explicitly unknown, never "probably shared" and never "probably whoever the label names". A reviewed decision can never regress to `unreconciled`; correcting one is a forward change recorded as a new version.

There is no fractional ownership, and its absence is structural rather than conventional. No share, weight, percentage, ratio or split column exists; the shape constraint gives `household` no room for a party; and representing shared ownership by writing an account once per party is exactly the duplication this contract forbids. Consequently party scopes deliberately do not reconstruct the household total — the shared account belongs to neither party — and `canonical_balance_sheet_v2` reports the shared and unreconciled counts so a party scope is honest about what it excludes rather than silently understating a position.

Ownership is not authorization, in either direction. `public.household_members` plus `private.is_household_member()` remains the only authorization root; no policy anywhere reads `ownership_kind` or `owner_party_id`; owning an account grants nothing and owning none takes nothing away. No API role may assign ownership, even though `authenticated` holds table-level `UPDATE` on `accounts` — the guard refuses it — and SHR-154 ships no ownership mutation API at all, because reconciliation is a reviewed release action rather than a product feature. The household-facing lifecycle belongs to SHR-158.

Evidence is domain state, not audit. Consistent with ADR-018, `049` writes no `audit_events` row and widens no audit constraint: an ownership decision's durable record is the append-only `account_ownership_history` row and the immutable run record. The account reference in that history is a typed logical reference rather than a foreign key, so deleting an account keeps working exactly as it does today while the evidence that its ownership was once decided outlives it.

The package is additive by construction. Every v1 canonical contract returns byte-identical results before and after, every pre-existing RLS policy is unchanged, `accounts` keeps its physical tuple identity across the migration, and no consumer is cut over — `owner` remains authoritative until SHR-173, SHR-153, SHR-172 and SHR-158 migrate their own surfaces under their own contracts.
