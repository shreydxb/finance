...
export function PrefsProvider({ children }) {
  ...
  useEffect(() => {
    getSetting('fx_rates')
      .then((rates) => rates && setFxRates(rates))
      .catch(() => {
        /* keep the AED-only fallback; screens still render, just unconverted */
      })
  }, [])
  ...
  const value = useMemo(() => {
    ...
    const fmt = (aed, opts) => formatMoney(fromAED(Number(aed) || 0, currency, fxRates), currency, opts)
    return { currency, setCurrency, theme, setTheme, fxRates, fmt }
  }, [currency, setCurrency, theme, setTheme, fxRates])
