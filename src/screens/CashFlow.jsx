...
import { sumByCategoryAED, totalAED } from '../lib/reports'
import {
  currentYearMonth,
  currentQuarter,
  monthRange,
...
function categoryGroups(transactions, fxRates) {
  const map = sumByCategoryAED(transactions, fxRates)
...
export default function CashFlow() {
  ...
  async function refresh() {
    setError('')
    try {
      const [txns, inc, fx, split] = await Promise.all([
  ...
  const catGroups = useMemo(() => categoryGroups(transactions, fxRates), [transactions, fxRates])
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-4">
        ...
        {breakdownView === 'category' ? (
          <BreakdownBars title="Expenses by category" groups={catGroups} formatValue={formatAED} emptyMessage="No expenses logged for this period." />
        ) : (
